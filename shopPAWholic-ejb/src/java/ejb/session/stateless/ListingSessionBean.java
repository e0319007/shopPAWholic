...
public class ListingSessionBean implements ListingSessionBeanLocal {
    ...
    private List<Listing> addSubCategoryListings(Category category) {
        List<Listing> listings = new ArrayList<>();
        
        if (category.getSubCategories().isEmpty()) {
            return category.getListings();
        } else {
            for (Category subCategory : category.getSubCategories()) {
                listings.addAll(addSubCategoryListings(subCategory));
            }
                ...
                if (listingToUpdate.getSkuCode().equals(listing.getSkuCode())) {
                    if (categoryId != null && (!listingToUpdate.getCategory().getCategoryId().equals(categoryId))) {
                        ...
                        if(!categoryToUpdate.getSubCategories().isEmpty()) {
                            throw new UpdateListingException("Selected category for new listing is not a leaf category!");
                        }
